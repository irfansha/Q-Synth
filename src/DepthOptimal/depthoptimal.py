...
from src.DepthOptimal.configs import (
...
from src.DepthOptimal.util.output_checker import (
    connectivity_check,
    equality_check,
)
from src.DepthOptimal.synthesizers.planning.solvers import OPTIMAL
...
import src.DepthOptimal.synthesizers.planning.solvers as planning
...
def depth_optimal_mapping(
    ...
    match output:
        case SynthesizerSolution():
            ...
            if verbose >= 0:
                print(
                    f"Completed mapping of {circuit_in} with {output.swaps} swaps and {output.cx_depth} cx-depth."
                )
            ...
            match (cx_optimal, swap_optimal):
                ...
                case False, True:  # depth-cx-count
                    opt_val = output.depth
                case True, False:  # cx-depth
                    opt_val = output.cx_depth
                case True, True:  # cx-depth-cx-count
                    opt_val = output.cx_depth

            # Return output circuit and optimisation value
            return output.mapped_circuit, opt_val
