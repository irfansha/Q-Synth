...
from src.CnotSynthesis.circuit_utils.clifford_circuit_utils import extract_circuit, compute_and_print_costs, compute_cost
...
def solve_cnot_synth(matrix, options, num_qubits):
   ...
   if options.verbose > 1: print("Solving time: " + str(solving_time))

   options.remaining_time = options.remaining_time - solving_time
   if options.verbose > 1: print("Remaining time: " + str(options.remaining_time))

   if solver.sat == -1:
     if options.verbose > 1: print("Timed out")
     return None, True
   elif solver.sat == 0:
     if options.verbose > 1: print("No plan found")
     return None, False
   elif solver.sat == 1:
     if options.verbose > 1: print("Plan found")
     # Extract the plan:
...
def set_options(encoding, solver, preprocessor, time, minimization, verbose, coupling_graph, plan_length, optimal_search, qubit_permute, intermediate_files_path,check_equivalence):
    options = op()
...
def cnot_optimization(circuit, encoding="lifted", solver="qubi", preprocessor='bloqqer', time=1800,
                      minimization='gates', verbose=0, coupling_graph=None, plan_length=None,
                      optimal_search=None, qubit_permute=False, intermediate_files_path="intermediate_files",check_equivalence=0):
    options = set_options(encoding, solver, preprocessor, time, minimization, verbose, coupling_graph, plan_length, optimal_search, qubit_permute, intermediate_files_path, check_equivalence)
    ...
    if (minimization == 'gates'):
      ...
      if verbose > 1: print("Original number of CNOTs : ", bound)
    else:
      bound = circuit.depth()
      if verbose > 1: print("Original circuit depth : ", bound)
    ...
    if (options.optimal_search == 'f'):
       for cur_plan_length in range(bound):
         ...
         if cur_opt_circuit != None:
           if (verbose > 1): print("Optimal solution found")
           opt_circuit = cur_opt_circuit
           break
         elif timed_out:
           if (verbose > 1): print("Timed out, search stopped")
           break
    ...
    if (opt_circuit != None):
      compute_and_print_costs(circuit, opt_circuit, cnot_minimization=minimization, verbose=options.verbose)
      if options.check_equivalence: equivalence_check(circuit, opt_circuit, None, options)
    return opt_circuit

# Optimize cnot circuit given a qasm file:
# Returns a more optimal circuit if found or returns the original circuit:
def cnot_optimization_from_file(circuit_in, encoding="lifted", solver="caqe", preprocessor='bloqqer', time=1800,
                                minimization='gates', verbose=0, coupling_graph=None, plan_length=None,
                                optimal_search=None, qubit_permute=False, intermediate_files_path="intermediate_files",check_equivalence=0):
    circuit = QuantumCircuit.from_qasm_file(circuit_in)
    return cnot_optimization(circuit, encoding, solver, preprocessor, time, minimization, verbose, coupling_graph, plan_length, optimal_search, qubit_permute, intermediate_files_path, check_equivalence)