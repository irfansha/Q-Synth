...
class GroundedSat:
  ...
  def generate_quantifier_blocks(self):
    # Header:
    self.quantifier_block.append('p cnf ' + str(self.encoding_variables.next_var - 1) + ' ' + str(len(self.clause_list)))
  ...
  def generate_transition_depth_optimal(self, time_step):
    ...
    for control_id in range(self.num_qubits):
      ...
      for cnot_varid in self.control_cnotvar_dict[control_id]:
        cur_conflicting_cnot_list.append(self.cnot_variables[time_step][cnot_varid])
    ...
    for cnot_varid in range(self.num_cnot_vars):
      ...
      if (self.options.coupling_graph  != None and not self.options.qubit_permute and [control_id, target_id] not in self.options.coupling_graph):
        self.constraints.unit_clause(-self.cnot_variables[time_step][cnot_varid])
        continue
      # If coupling graph is given and qubit permutation is enable
      # then we disable permuted unconnected cnot operations:
      elif(self.options.coupling_graph  != None and self.options.qubit_permute and [control_id, target_id] not in self.options.coupling_graph):
        ...
        for permuted_cid in range(self.num_qubits):
          for permuted_tid in range(self.num_qubits):
            if ((permuted_cid, permuted_tid) in self.reverse_control_target_dict):
              permuted_cnotvarid = self.reverse_control_target_dict[(permuted_cid, permuted_tid)]
              self.constraints.or_clause([-self.cells[initial_timestep][control_id][permuted_cid], -self.cells[initial_timestep][target_id][permuted_tid],
                                          -self.cnot_variables[time_step][permuted_cnotvarid]])
      # if imply touched variables for propagation:
      self.constraints.if_then_clause(self.cnot_variables[time_step][cnot_varid],[self.touched_qvars[time_step][target_id]])
      ...
      for row_id in range(self.num_qubits):
        ...
        self.constraints.if_and_then_notequal_clauses([self.cnot_variables[time_step][cnot_varid],self.cells[time_step][row_id][control_id]],self.cells[time_step][row_id][target_id],
        ...
        self.constraints.if_and_then_equal_clauses([self.cnot_variables[time_step][cnot_varid],-self.cells[time_step][row_id][control_id]],self.cells[time_step][row_id][target_id],
                                                    self.cells[time_step+1][row_id][target_id])
    ...
    for target_id in range(self.num_qubits):
      ...
      self.constraints.if_then_clause(self.touched_qvars[time_step][target_id],cur_cnot_vars)

    # if not touched then cells are propagated to the next time step:
    for column_id in range(self.num_qubits):
      # forall rows, we duplicate the constraints:
      for row_id in range(self.num_qubits):
        self.constraints.if_and_then_equal_clauses([-self.touched_qvars[time_step][column_id]],self.cells[time_step][row_id][column_id], self.cells[time_step+1][row_id][column_id])
  ...
  def generate_transition_cnot_optimal(self, time_step):
      ...
      for control_id in range(self.num_qubits):
        for target_id in range(self.num_qubits):
          if ([control_id, target_id] not in self.options.coupling_graph):
            self.constraints.or_clause([-self.control_qvars[time_step][control_id], -self.target_qvars[time_step][target_id]])
    # If coupling graph is given and qubit permutation is enable
    # then we disable permuted unconnected cnot operations:
        ...
        for target_id in range(self.num_qubits):
          if ([control_id, target_id] not in self.options.coupling_graph):
            ...
            for permuted_cid in range(self.num_qubits):
              for permuted_tid in range(self.num_qubits):
                   self.constraints.or_clause([-self.cells[initial_timestep][control_id][permuted_cid], -self.cells[initial_timestep][target_id][permuted_tid],
                                               -self.control_qvars[time_step][permuted_cid], -self.target_qvars[time_step][permuted_tid]])
    else:
      ...
      for qid in range(self.num_qubits):
        self.constraints.or_clause([-self.control_qvars[time_step][qid], -self.target_qvars[time_step][qid]])
    ...
    for control_id in range(self.num_qubits):
      for target_id in range(self.num_qubits):
        ...
        for row_id in range(self.num_qubits):
          ...
          self.constraints.if_and_then_notequal_clauses([self.control_qvars[time_step][control_id], self.target_qvars[time_step][target_id],self.cells[time_step][row_id][control_id]],
                                                         self.cells[time_step][row_id][target_id],self.cells[time_step+1][row_id][target_id])
          # if the control and target qubits are true, and control cell is false, then the target cell is propagated:
          self.constraints.if_and_then_equal_clauses([self.control_qvars[time_step][control_id], self.target_qvars[time_step][target_id],-self.cells[time_step][row_id][control_id]],
                                                      self.cells[time_step][row_id][target_id],self.cells[time_step+1][row_id][target_id])
    ...
    for column_id in range(self.num_qubits):
      # Forall rows, we add the cell update clauses:
      for row_id in range(self.num_qubits):
        self.constraints.if_and_then_equal_clauses([-self.target_qvars[time_step][column_id]],
                                                    self.cells[time_step][row_id][column_id],self.cells[time_step+1][row_id][column_id])
  ...
  def generate_goal_gate(self):
    ...
    for row_id in range(self.num_qubits):
      for column_id in range(self.num_qubits):
        ...
        if (cell == True):
          self.constraints.unit_clause(self.cells[time_step][row_id][column_id])
        else:
          self.constraints.unit_clause(-self.cells[time_step][row_id][column_id])
  ...
  def __init__(self, matrix, options, num_qubits, verbose=0):
    ...
    if (self.options.minimization == 'depth'):
      ...
      for i in range(self.options.plan_length):
        self.cnot_variables.append(self.encoding_variables.get_vars(self.num_cnot_vars))
        self.touched_qvars.append(self.encoding_variables.get_vars(self.num_qubits))
      ...
      for control_id in range(self.num_qubits):
        for target_id in range(self.num_qubits):
          if control_id != target_id:
            ...
            self.reverse_control_target_dict[(control_id, target_id)] = counter
            self.initialize_or_add_element(self.control_cnotvar_dict, control_id, counter)
            self.initialize_or_add_element(self.target_cnotvar_dict, target_id, counter)
            counter = counter + 1
      ...
      if (self.options.verbose > 2):
        ...
        print("Reverse Control-Target Dict Variables: ",self.reverse_control_target_dict)
    ...
    for time_step in range(self.options.plan_length+1):
      single_row_cells = []
      for row_id in range(self.num_qubits):
        single_row_cells.append(self.encoding_variables.get_vars(self.num_qubits))
