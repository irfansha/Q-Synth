...
class LiftedQbf:
  ...
  def generate_quantifier_blocks(self):
    ...
    if self.options.minimization == 'depth':
      ...
      for i in range(self.options.plan_length):
        self.quantifier_block.append('e ' + ' '.join(str(x) for x in self.cnot_variables[i]) + ' 0')
        self.quantifier_block.append('e ' + ' '.join(str(x) for x in self.touched_qvars[i]) + ' 0')
    else:
      assert(self.options.minimization == 'gates')
      # Control and target cnot variables:
      for i in range(self.options.plan_length):
        self.quantifier_block.append('e ' + ' '.join(str(x) for x in self.control_qvars[i]) + ' 0')
        self.quantifier_block.append('e ' + ' '.join(str(x) for x in self.target_qvars[i]) + ' 0')
    # Forall row variables:
    self.quantifier_block.append('a ' + ' '.join(str(x) for x in self.forall_rows) + ' 0')
    # Exists row variables:
    self.quantifier_block.append('e ' + ' '.join(str(x) for x in self.exists_rows) + ' 0')
    # Cell variables:
    for i in range(self.options.plan_length+1):
      self.quantifier_block.append('e ' + ' '.join(str(x) for x in self.cells[i]) + ' 0')
  ...
  def generate_transition_depth_optimal(self, time_step):
    ...
    for cnot_varid in range(self.num_cnot_vars):
      ...
      if (self.options.coupling_graph  != None and [control_id, target_id] not in self.options.coupling_graph):
      ...
      self.constraints.if_and_then_notequal_clauses([self.cnot_variables[time_step][cnot_varid],self.cells[time_step][control_id]],self.cells[time_step][target_id],self.cells[time_step+1][target_id])
      # for each cnot var, if the control qubit is false then the target qubit is propagated:
      self.constraints.if_and_then_equal_clauses([self.cnot_variables[time_step][cnot_varid],-self.cells[time_step][control_id]],self.cells[time_step][target_id],self.cells[time_step+1][target_id])
    ...
    for column_id in range(self.num_qubits):
      self.constraints.if_and_then_equal_clauses([-self.touched_qvars[time_step][column_id]],self.cells[time_step][column_id], self.cells[time_step+1][column_id])
  ...
  def generate_transition_cnot_optimal(self, time_step):
    ...
    for control_id in range(self.num_qubits):
      for target_id in range(self.num_qubits):
        ...
        self.constraints.if_and_then_notequal_clauses([self.control_qvars[time_step][control_id], self.target_qvars[time_step][target_id],self.cells[time_step][control_id]],
                                                       self.cells[time_step][target_id],self.cells[time_step+1][target_id])
        # if the control and target qubits are true, and control cell is false, then the target cell is propagated:
        self.constraints.if_and_then_equal_clauses([self.control_qvars[time_step][control_id], self.target_qvars[time_step][target_id],-self.cells[time_step][control_id]],
                                                    self.cells[time_step][target_id],self.cells[time_step+1][target_id])
    ...
    for column_id in range(self.num_qubits):
      self.constraints.if_and_then_equal_clauses([-self.target_qvars[time_step][column_id]],
                                                  self.cells[time_step][column_id],self.cells[time_step+1][column_id])
  def generate_initial_gate(self):
    ...
    for row_id in range(self.num_qubits):
      self.constraints.eq_clause(self.exists_rows[row_id], self.cells[time_step][row_id])
  ...
  def generate_goal_gate(self):
    ...
    for row_id in range(self.num_qubits):
      for column_id in range(self.num_qubits):
        ...
        if (cell == True):
          self.constraints.if_then_clause(self.exists_rows[row_id],[self.cells[time_step][column_id]])
        else:
          self.constraints.if_then_clause(self.exists_rows[row_id],[-self.cells[time_step][column_id]])
