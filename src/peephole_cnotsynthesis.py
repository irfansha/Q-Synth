...
from src.PeepholeSlicing.circuit_utils import remove_zero_cost_swaps, equivalence_check_with_map
from src.CnotSynthesis.cnot_synthesis import cnot_optimization as cnot_op
from src.CnotSynthesis.cnot_synthesis_sat_qbf import cnot_optimization as cnot_op_sat_qbf
from src.CnotSynthesis.circuit_utils.clifford_circuit_utils import equivalence_check_measureall, compute_cost, compute_cnot_cost, compute_cnot_depth, compute_depth_swaps_as_3cx, compute_cnotdepth_swaps_as_3cx, compute_and_print_costs
...
def peephole_cnotsynthesis(circuit_in=None, circuit_out=None, minimize="gates", model="sat", qubit_permute=None,
  ...
  # ----------------------------------------------------------------------------------------------------
    ...
    if args.circuit_in == None:
      args.circuit_in = os.path.join(args.benchmarks,"ECAI-24","tpar-optimized","barenco_tof_3.qasm")


    # if platform is chosen then coupling graph is extracted:
    if args.platform:
      coupling_graph = pt(args.platform, args.bidirectional)[1]
    else:
      coupling_graph = None
    ...
    if (args.model == "planning"):
      if args.solver == None:
        ...
        assert(args.solver in ["fd-ms", "lama"])
    ...
    if (args.qubit_permute):
      assert args.sat_solver != None, "qubit permutation only available with sat encoding, please use a sat solver or turnoff the permutation option"
    ...
    sliced_circuit = cu(circuit_copy, "cnot", args.check_equivalence)
    ...
    for slice_id in range(total_slices):
      ...
      if args.verbose >= 0: print(f"Solving slice {slice_id+1} of {total_slices} ...")
      ...
      if num_cx_gates < 1: cur_opt_circuit = None
        ...
        if (args.planner != None):
          ...
          cur_opt_circuit = cnot_op_sat_qbf(slice.optimization_slice, solver=solver, preprocessor="bloqqer", time=args.time,
                                            minimization=args.minimize, verbose=args.verbose, coupling_graph=coupling_graph, optimal_search="f",
                                            qubit_permute=args.qubit_permute, intermediate_files_path=args.intermediate_files_path, check_equivalence=args.check_equivalence)
        else:
      ...
      if (cur_opt_circuit == None):
        ...
        initial_cost, initial_cx_gates, final_cost, final_cx_gates = compute_and_print_costs(slice.optimization_slice, cur_opt_circuit, cnot_minimization=args.minimize, verbose=0)
        ...
        if (args.minimize == 'depth' and
            compute_depth_swaps_as_3cx(slice.optimization_slice) > compute_depth_swaps_as_3cx(cur_opt_circuit)):
    ...
    if (args.qubit_permute):
      ...
      if args.check_equivalence:
        ...
        equivalence_check_with_map(sliced_circuit.measurementless_circuit, opt_circuit, post_mapping,verbose=args.verbose)
    else:
      ...
      if args.verbose > 0:
        ...
        print(opt_circuit)
      if args.check_equivalence:
        print("\nChecking Equivalence between original and optimized circuit")
        equivalence_check_measureall(circuit, opt_circuit,verbose=True)
    ...
    print(f"\tCNOT depth: {compute_cnot_depth(circuit)} -> {compute_cnot_depth(opt_circuit)}")
    print(f"\tNo. of CNOTs: {compute_cnot_cost(circuit)} -> {compute_cnot_cost(opt_circuit)}")
    print("\nMetrics when SWAP gates are considered as 3 CNOTs:")
    print(f"\tDepth : {compute_depth_swaps_as_3cx(circuit, args.check_equivalence, args.verbose)} -> {compute_depth_swaps_as_3cx(opt_circuit, args.check_equivalence, args.verbose)}")
    print(f"\tCNOT depth: {compute_cnotdepth_swaps_as_3cx(circuit)} -> {compute_cnotdepth_swaps_as_3cx(opt_circuit)}")
    print(f"\tNo. of CNOTs: {compute_cost(circuit)[1]} -> {compute_cost(opt_circuit)[1]}")


    if (args.circuit_out != None):
      with open(args.circuit_out, "w") as f:
        f.write(QuantumCircuit.qasm(opt_circuit))