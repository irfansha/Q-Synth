...
; we restrict the ancillary swaps, to initially map unused lqubits
; (so strictly speaking, these are not using ancillary pqubits)

(:action swap10 ; with implicit initial mapping
    :parameters (?l1 ?l2 - lbit ?p1 ?p2 - pbit)
    :precondition (and 
        (connected ?p1 ?p2)
        (mapped ?l1 ?p1)
    ...
    :effect (and 
        (not (mapped ?l1 ?p1)) (done ?l2)
        (mapped ?l1 ?p2) (mapped ?l2 ?p1) (occupied ?p2)
    )    
)

(:action swap01 ; with implicit initial mapping
    :parameters (?l1 ?l2 - lbit ?p1 ?p2 - pbit)
    :precondition (and 
        (connected ?p1 ?p2)
        (mapped ?l2 ?p2)
    ...
    :effect (and 
        (not (mapped ?l2 ?p2)) (done ?l1)
        (mapped ?l2 ?p1) (mapped ?l1 ?p2) (occupied ?p1)
    )    
)
