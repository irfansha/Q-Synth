...
Path_to_standard_benchmarks="../../Benchmarks/SAT-24/Standard/"
Path_to_vqe_benchmarks="../../Benchmarks/SAT-24/VQE/"
...
for bridge_relaxed_configuration in "${Bridge_Relaxed_configurations[@]}"; do
  ...
  for file in ${Standard_instances}; do
    echo -e "\nCircuit: "$file
    ../../q-synth.py layout --bridge ${brconf[1]} -r ${brconf[0]} -m sat -p melbourne -s cd15 -v1 -t 600 $Path_to_standard_benchmarks$file
  done
done
echo "VQE instances"
for bridge_relaxed_configuration in "${Bridge_Relaxed_configurations[@]}"; do
  ...
  for file in ${VQE_instances}; do
    echo -e "\nCircuit: "$file
    ../../q-synth.py layout --bridge ${brconf[1]} -r ${brconf[0]} -m sat -p melbourne -s cd15 -v1 -t 600 $Path_to_vqe_benchmarks$file
  done
done