...
for platform_name_and_option in "${Platform_name_and_option[@]}"; do
  ...
  for metric_combination in "${Metric_combinations[@]}"; do
    ...
    for file in ${VQE_mapped_instances}; do
      echo -e "\nCircuit: "$file
      ../../q-synth.py clifford --solver cd ${metric_combination} -t 600 -v 0 ../../Benchmarks/VQE/mapped${platformop[0]}/$file -p ${platformop[1]}
    done
  done
done
...
for platform_name_and_option in "${Platform_name_and_option[@]}"; do
  ...
  for metric_combination in "${Metric_combinations[@]}"; do
    ...
    for file in ${Feynman_mapped_instances}; do
      echo -e "\nCircuit: "$file
      ../../q-synth.py clifford --solver cd ${metric_combination} -t 600 -v 0 ../../Benchmarks/Feynman/mapped${platformop[0]}/$file -p ${platformop[1]}
    done
  done
done