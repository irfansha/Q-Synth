...
class TestingMappedCircuit:
    ...
    def test_mapped_circuit(self):
        ...
        if self.args.relaxed == 0 and self.args.cnot_cancel == 0 and self.args.bidirectional < 2 and self.args.bridge == 0: 
          # TODO: and self.args.qiskit_optimization == 0:
          self.compare_layers()
          if self.args.verbose > -1:
            print("Assertions complete, mapped circuit has the same layers as the original one")
        else:
          compare = qcec.verify(self.input_circuit, self.unmapped_circuit)
          if self.args.verbose > -1:
            print("Compare input with back-mapped output using mqt.qcec: ",compare.equivalence)
          assert(str(compare.equivalence) == 'equivalent')
